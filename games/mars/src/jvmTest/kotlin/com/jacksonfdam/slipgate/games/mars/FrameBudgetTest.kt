...
/**
 * What one frame of Doom costs on this host, which is the frame budget the README documents.
 *
 ...
 *
 * ```
 * ./gradlew :games:mars:jvmTest --tests '*FrameBudgetTest*' -Pslipgate.iwad=/path/to/freedoom1.wad
 * ```
 */
        ...
        runTest {
            val file =
                iwad?.takeIf { it.isFile } ?: return@runTest run {
                    println("skipping: set -Pslipgate.iwad to a doom IWAD to measure the frame budget")
                }
            val factory = assertNotNull(gate.sessionFactories()[BackendId.Wasm])
            val session = factory.create(SingleFileData(MARS_IWAD, file.readBytes()), RecordingHost())
            ...
            println(
                "Doom frame budget: median $median ms, 99th $ninetyNinth ms, worst $worst ms",
            )
