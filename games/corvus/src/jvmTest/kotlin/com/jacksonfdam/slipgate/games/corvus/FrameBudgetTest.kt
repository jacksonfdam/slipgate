...
/**
 * What one frame of Heretic costs on this host, which is the frame budget the README documents.
 *
 ...
 *
 * ```
 * ./gradlew :games:corvus:jvmTest --tests '*FrameBudgetTest*' -Pslipgate.iwad=/path/to/heretic.wad
 * ```
 */
        ...
        runTest {
            val file =
                iwad?.takeIf { it.isFile } ?: return@runTest run {
                    println("skipping: set -Pslipgate.iwad to a heretic IWAD to measure the frame budget")
                }
            val factory = assertNotNull(gate.sessionFactories()[BackendId.Wasm])
            val host = RecordingHost()
            val session = factory.create(SingleFileData(CORVUS_IWAD, file.readBytes()), host)

            repeat(WARMUP_FRAMES) { frame ->
                runCatching { session.step(InputFrame.Idle, TIC_MILLIS) }
                    .onFailure { failure -> error("warmup frame $frame: ${failure.message}\n${host.tail(20)}") }
            }
            ...
            for (frame in 0 until MEASURED_FRAMES) {
                ...
                val result =
                    runCatching { session.step(InputFrame.Idle, TIC_MILLIS) }
                        .getOrElse { failure -> error("measured frame $frame: ${failure.message}\n${host.tail(20)}") }
            ...
            println(
                "Heretic frame budget: median $median ms, 99th $ninetyNinth ms, worst $worst ms",
            )
